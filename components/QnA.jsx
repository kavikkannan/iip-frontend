...
const QuestionPage = () => {
  ...
  const [submissionStatus, setSubmissionStatus] = useState(null); // 'processing' | 'done' | null
  const [answers, setAnswers] = useState({});
  const videoRef = useRef(null);
  const mediaRecorderRef = useRef(null);
  const streamRef = useRef(null);
  const sampleVideoUrl = "/sample-video.mp4"; // Replace with actual video URL
  ...
  const calculateScore = () => {
    const scoringIndexes = [1, 7, 8, 10]; // 1-based index for scoring 1 point if Agree
    let scoreObject = {};
    ...
    Object.entries(answers).forEach(([key, value]) => {
        ...
        if (scoringIndexes.includes(questionIndex) && value.includes("Agree")) {
            score = 1;
        } else if (!scoringIndexes.includes(questionIndex) && value.includes("Disagree")) {
            score = 1;
        }

        scoreObject[scoreKey] = score; // Store in a single object
    });
...
const handleSubmit = async () => {
  ...
  const scores = calculateScore(); // Get scores as an object
  ...
  // Convert recordedChunks (Blob array) to a video file
  ...
  Object.entries(userData).forEach(([key, value]) => {
      if (value !== null) {
          formData.append(key, parseInt(value, 10)); // Convert to integer
      }
  });
  ...
  try {
    ...
    try {
      ...
      const std_info = JSON.parse(sessionStorage.getItem("studentInfo")); // ✅ Parse stored JSON
      
      console.log("Prediction Result:", jsonResponse.prediction);
      
      let asd_confidence = (jsonResponse.asd_confidence * 100).toFixed(2); // ✅ Convert to string
      ...
      try {
          const response = await fetch("http://127.0.0.1:9999/setReport", { 
              ...
              body: JSON.stringify({
                  name: String(std_info.name),       // ✅ Ensure strings
        ...
        setSubmissionStatus('done'); // Show success message
    } catch (error) {
    ...
    <div className="h-screen bg-black text-white">
      ...
      {!showQuestions ? (
        <div className="flex flex-col items-center justify-center h-full">
          <video ref={videoRef} autoPlay muted className="w-full  max-w-lg border-2 border-white" />
          <video
        autoPlay
        muted
        loop
      ...
      >
          ...
          {!recording && recordedChunks.length > 0 && (
            <button onClick={handleNextStep} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">Next Step</button>
          )}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <div className="mt-4 flex space-x-4">
            <button onClick={() => setSelectedQuestion(Math.max(1, selectedQuestion - 1))} className="px-4 py-2 bg-gray-500 text-white rounded" disabled={selectedQuestion === 1}>Previous</button>
            <button onClick={() => setSelectedQuestion(Math.min(questions.length, selectedQuestion + 1))} className="px-4 py-2 bg-gray-500 text-white rounded" disabled={selectedQuestion === questions.length}>Next</button>
          </div>
          {selectedQuestion === questions.length && (
            ...
            <button onClick={handleSubmit} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">Submit</button>
          ) : (
            <button onClick={handleSubmit} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">Submit</button>
          )}
            </div>
